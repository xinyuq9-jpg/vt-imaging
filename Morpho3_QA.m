...
for iFile = 1:length(list_input_files) % loop through video files
    ...
    % read video metadata for context frame navigation
    v_qa = VideoReader(input_avi_filename);
    no_video_frames = floor(v_qa.Duration * v_qa.FrameRate);

    if cfg.qa.context_frame_chooser == "auto"
        context_frame = round(no_video_frames / 2);
    else
        context_frame = cfg.qa.context_frame_default;
    end

    % read image data (streaming stats for mean anatomy frame)
    ...
    if shared_QA_mask_loaded && cfg.qa.skip_manual_QA_if_shared_mask_loaded == 1
        ...
        editor = morpho.Editor(vt_ever, ...
            ...
            frame_indices=1:no_video_frames, ...
